...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/schools/:id", requireRole(["super_admin", "school_admin"]), async (req, res) => {
    try {
  ...
  // Update a school
  app.put("/api/schools/:id", requireRole(["super_admin", "school_admin"]), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app.get("/api/schools/:schoolId/admins", requireRole(["super_admin"]), async (req, res) => {
    try {
      const schoolId = parseInt(req.params.schoolId);
      const admins = await storage.getSchoolAdminsBySchoolId(schoolId);
      res.json(admins);
    } catch (error) {
      res.status(500).json({ message: "Failed to fetch school admins", error });
    }
  });
  
  // Create a school admin
  app.post("/api/school-admins", requireRole(["super_admin"]), async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to create school admin", error });
    }
  });
  
  // Update a school admin
  app.put("/api/school-admins/:id", requireRole(["super_admin"]), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      ...
      res.status(500).json({ message: "Failed to update school admin", error });
    }
  });
  
  // Delete a school admin
  app.delete("/api/school-admins/:id", requireRole(["super_admin"]), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      ...
      res.status(500).json({ message: "Failed to delete school admin", error });
    }
  });
  ...
  // Update a teacher
  app.put("/api/teachers/:id", requireRole(["super_admin", "school_admin"]), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  // Delete a teacher
  app.delete("/api/teachers/:id", requireRole(["super_admin", "school_admin"]), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app.get("/api/schools/:schoolId/students", requireRole(["super_admin", "school_admin", "teacher"]), async (req, res) => {
    try {
      const schoolId = parseInt(req.params.schoolId);
  ...
  // Update a student
  app.put("/api/students/:id", requireRole(["super_admin", "school_admin"]), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  // Delete a student
  app.delete("/api/students/:id", requireRole(["super_admin", "school_admin"]), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  // Create a class
  app.post("/api/classes", requireRole(["super_admin", "school_admin"]), async (req, res) => {
    try {
  ...
  // Update a class
  app.put("/api/classes/:id", requireRole(["super_admin", "school_admin"]), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  // Delete a class
  app.delete("/api/classes/:id", requireRole(["super_admin", "school_admin"]), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app.get("/api/schools/:schoolId/subjects", requireRole(["super_admin", "school_admin", "teacher"]), async (req, res) => {
    try {
      const schoolId = parseInt(req.params.schoolId);
  ...
  // Create a subject
  app.post("/api/subjects", requireRole(["super_admin", "school_admin"]), async (req, res) => {
    try {
  ...
  // Update a subject
  app.put("/api/subjects/:id", requireRole(["super_admin", "school_admin"]), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  // Delete a subject
  app.delete("/api/subjects/:id", requireRole(["super_admin", "school_admin"]), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app.post("/api/class-subjects", requireRole(["super_admin", "school_admin"]), async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to create class-subject mapping", error });
    }
  });
  
  // Get subjects for a class
  app.get("/api/classes/:classId/subjects", requireRole(["super_admin", "school_admin", "teacher", "student", "parent"]), async (req, res) => {
    try {
      const classId = parseInt(req.params.classId);
      const mappings = await storage.getClassSubjectsByClassId(classId);
      res.json(mappings);
    } catch (error) {
      res.status(500).json({ message: "Failed to fetch class subjects", error });
    }
  });
  
  // Update a class-subject mapping
  app.put("/api/class-subjects/:id", requireRole(["super_admin", "school_admin"]), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const mappingData = insertClassSubjectSchema.partial().parse(req.body);
      
      const updatedMapping = await storage.updateClassSubject(id, mappingData);
      if (!updatedMapping) {
      ...
      res.status(500).json({ message: "Failed to update class-subject mapping", error });
    }
  });
  ...
  app.post("/api/student-attendance", requireRole(["school_admin", "teacher"]), async (req, res) => {
    try {
      const attendanceData = insertStudentAttendanceSchema.parse(req.body);
      const newAttendance = await storage.createStudentAttendance(attendanceData);
  ...
  app.get("/api/students/:studentId/attendance", requireRole(["super_admin", "school_admin", "teacher", "student", "parent"]), async (req, res) => {
    try {
      const studentId = parseInt(req.params.studentId);
      const attendance = await storage.getStudentAttendanceByStudentId(studentId);
      res.json(attendance);
    } catch (error) {
  ...
  app.get("/api/classes/:classId/attendance", requireRole(["super_admin", "school_admin", "teacher"]), async (req, res) => {
    try {
      const classId = parseInt(req.params.classId);
      const dateStr = req.query.date as string;
      const date = dateStr ? new Date(dateStr) : new Date();
      
      const attendance = await storage.getStudentAttendanceByClassId(classId, date);
      res.json(attendance);
    } catch (error) {
      res.status(500).json({ message: "Failed to fetch class attendance", error });
    }
  });
  ...
  // Create an exam
  app.post("/api/exams", requireRole(["super_admin", "school_admin"]), async (req, res) => {
    try {
  ...
  // Get exams by school
  app.get("/api/schools/:schoolId/exams", requireRole(["super_admin", "school_admin", "teacher", "student", "parent"]), async (req, res) => {
    try {
      const schoolId = parseInt(req.params.schoolId);
  ...
  // Get exams by class
  app.get("/api/classes/:classId/exams", requireRole(["super_admin", "school_admin", "teacher", "student", "parent"]), async (req, res) => {
    try {
      const classId = parseInt(req.params.classId);
  ...
  app.post("/api/fee-structures", requireRole(["super_admin", "school_admin"]), async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to create fee structure", error });
    }
  });
  
  // Get fee structures by class
  app.get("/api/classes/:classId/fee-structures", requireRole(["super_admin", "school_admin", "teacher", "student", "parent"]), async (req, res) => {
    try {
      const classId = parseInt(req.params.classId);
      const feeStructures = await storage.getFeeStructuresByClassId(classId);
      res.json(feeStructures);
    } catch (error) {
      res.status(500).json({ message: "Failed to fetch fee structures", error });
    }
  });
  
  // Record fee payment
  app.post("/api/fee-payments", requireRole(["super_admin", "school_admin"]), async (req, res) => {
    try {
  ...
  app.get("/api/students/:studentId/fee-payments", requireRole(["super_admin", "school_admin", "teacher", "student", "parent"]), async (req, res) => {
    try {
      const studentId = parseInt(req.params.studentId);
      const payments = await storage.getFeePaymentsByStudentId(studentId);
      res.json(payments);
    } catch (error) {
      res.status(500).json({ message: "Failed to fetch fee payments", error });
    }
  });
  ...
  // Create a bill
  app.post("/api/bills", requireRole(["super_admin", "school_admin"]), async (req, res) => {
    try {
  ...
  // Get bills by school
  app.get("/api/schools/:schoolId/bills", requireRole(["super_admin", "school_admin"]), async (req, res) => {
    try {
      const schoolId = parseInt(req.params.schoolId);
  ...
  // Update bill status
  app.put("/api/bills/:id", requireRole(["super_admin", "school_admin"]), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  // Send a message
  app.post("/api/messages", requireRole(["super_admin", "school_admin", "teacher"]), async (req, res) => {
    try {
  ...
  app.get("/api/messages/received", requireRole(["super_admin", "school_admin", "teacher", "student", "parent"]), async (req, res) => {
    try {
      const userId = (req.user as Express.User).id;
      const userRole = (req.user as Express.User).role;
      
      const messages = await storage.getMessagesByReceiverId(userId, userRole);
      res.json(messages);
    } catch (error) {
      res.status(500).json({ message: "Failed to fetch messages", error });
    }
  ...
  app.get("/api/messages/sent", requireRole(["super_admin", "school_admin", "teacher", "student", "parent"]), async (req, res) => {
    try {
      const userId = (req.user as Express.User).id;
  ...
  app.post("/api/class-messages", requireRole(["school_admin", "teacher"]), async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to send class message", error });
    }
  });
  
  // Get messages for a class
  app.get("/api/classes/:classId/messages", requireRole(["school_admin", "teacher", "student", "parent"]), async (req, res) => {
    try {
      const classId = parseInt(req.params.classId);
      const messages = await storage.getClassMessagesByClassId(classId);
      res.json(messages);
    } catch (error) {
      res.status(500).json({ message: "Failed to fetch class messages", error });
    }
  });
