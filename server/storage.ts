...
export class DatabaseStorage implements IStorage {
  ...
  // SchoolAdmin operations
  async getSchoolAdmin(id: number): Promise<SchoolAdmin | undefined> {
    const [admin] = await db.select().from(schoolAdmins).where(eq(schoolAdmins.id, id));
    return admin;
  }
  ...
  async createSchoolAdmin(insertSchoolAdmin: InsertSchoolAdmin): Promise<SchoolAdmin> {
    const [admin] = await db.insert(schoolAdmins).values(insertSchoolAdmin).returning();
    return admin;
  }
  ...
  async deleteSchoolAdmin(id: number): Promise<boolean> {
    const [deleted] = await db.delete(schoolAdmins)
      .where(eq(schoolAdmins.id, id))
      .returning();
    return !!deleted;
  }
  ...
  async deleteTeacher(id: number): Promise<boolean> {
    const [deleted] = await db.delete(teachers)
      .where(eq(teachers.id, id))
      .returning();
    return !!deleted;
  }
  ...
  async createClass(insertClass: InsertClass): Promise<Class> {
    const [classItem] = await db.insert(classes).values(insertClass).returning();
    return classItem;
  }
  ...
  // ClassSubject operations
  async getClassSubject(id: number): Promise<ClassSubject | undefined> {
    const [classSubject] = await db.select().from(classSubjects).where(eq(classSubjects.id, id));
    return classSubject;
  }
  ...
  async createClassSubject(insertClassSubject: InsertClassSubject): Promise<ClassSubject> {
    const [classSubject] = await db.insert(classSubjects).values(insertClassSubject).returning();
    return classSubject;
  }

  async updateClassSubject(id: number, data: Partial<InsertClassSubject>): Promise<ClassSubject | undefined> {
    const [updated] = await db.update(classSubjects)
      .set(data)
  ...
  async deleteClassSubject(id: number): Promise<boolean> {
    const [deleted] = await db.delete(classSubjects)
      .where(eq(classSubjects.id, id))
      .returning();
    return !!deleted;
  }
  ...
  async createStudentAttendance(insertAttendance: InsertStudentAttendance): Promise<StudentAttendance> {
    const [attendance] = await db.insert(studentAttendance).values(insertAttendance).returning();
    return attendance;
  }
  ...
  // TeacherAttendance operations
  async getTeacherAttendance(id: number): Promise<TeacherAttendance | undefined> {
    const [attendance] = await db.select().from(teacherAttendance).where(eq(teacherAttendance.id, id));
    return attendance;
  }
  ...
  async createTeacherAttendance(insertAttendance: InsertTeacherAttendance): Promise<TeacherAttendance> {
    const [attendance] = await db.insert(teacherAttendance).values(insertAttendance).returning();
    return attendance;
  }
  ...
  async deleteTeacherAttendance(id: number): Promise<boolean> {
    const [deleted] = await db.delete(teacherAttendance)
      .where(eq(teacherAttendance.id, id))
      .returning();
    return !!deleted;
  }
  ...
  async createLessonPlan(insertLessonPlan: InsertLessonPlan): Promise<LessonPlan> {
    const [plan] = await db.insert(lessonPlans).values(insertLessonPlan).returning();
    return plan;
  }

  async updateLessonPlan(id: number, data: Partial<InsertLessonPlan>): Promise<LessonPlan | undefined> {
    const [updated] = await db.update(lessonPlans)
      .set(data)
  ...
  async deleteLessonPlan(id: number): Promise<boolean> {
    const [deleted] = await db.delete(lessonPlans)
      .where(eq(lessonPlans.id, id))
      .returning();
    return !!deleted;
  }
  ...
  async deleteAssignmentSubmission(id: number): Promise<boolean> {
    const [deleted] = await db.delete(assignmentSubmissions)
      .where(eq(assignmentSubmissions.id, id))
      .returning();
    return !!deleted;
  }
  ...
  // ExamSubject operations
  async getExamSubject(id: number): Promise<ExamSubject | undefined> {
    const [examSubject] = await db.select().from(examSubjects).where(eq(examSubjects.id, id));
    return examSubject;
  }
  ...
  async createExamSubject(insertExamSubject: InsertExamSubject): Promise<ExamSubject> {
    const [examSubject] = await db.insert(examSubjects).values(insertExamSubject).returning();
    return examSubject;
  }

  async updateExamSubject(id: number, data: Partial<InsertExamSubject>): Promise<ExamSubject | undefined> {
    const [updated] = await db.update(examSubjects)
      .set(data)
  ...
  async deleteExamSubject(id: number): Promise<boolean> {
    const [deleted] = await db.delete(examSubjects)
      .where(eq(examSubjects.id, id))
      .returning();
    return !!deleted;
  }
  ...
  // FeeStructure operations
  async getFeeStructure(id: number): Promise<FeeStructure | undefined> {
    const [feeStructure] = await db.select().from(feeStructures).where(eq(feeStructures.id, id));
    return feeStructure;
  }
  ...
  async deleteFeeStructure(id: number): Promise<boolean> {
    const [deleted] = await db.delete(feeStructures)
      .where(eq(feeStructures.id, id))
      .returning();
    return !!deleted;
  }
  ...
  // FeePayment operations
  async getFeePayment(id: number): Promise<FeePayment | undefined> {
    const [feePayment] = await db.select().from(feePayments).where(eq(feePayments.id, id));
    return feePayment;
  }
  ...
  async createFeePayment(insertFeePayment: InsertFeePayment): Promise<FeePayment> {
    const [feePayment] = await db.insert(feePayments).values(insertFeePayment).returning();
    return feePayment;
  }
  ...
  // ClassMessage operations
  async getClassMessage(id: number): Promise<ClassMessage | undefined> {
    const [message] = await db.select().from(classMessages).where(eq(classMessages.id, id));
    return message;
  }
  ...
  async createClassMessage(insertClassMessage: InsertClassMessage): Promise<ClassMessage> {
    const [message] = await db.insert(classMessages).values(insertClassMessage).returning();
    return message;
  }

  async updateClassMessage(id: number, data: Partial<InsertClassMessage>): Promise<ClassMessage | undefined> {
    const [updated] = await db.update(classMessages)
      .set(data)
  ...
  async deleteClassMessage(id: number): Promise<boolean> {
    const [deleted] = await db.delete(classMessages)
      .where(eq(classMessages.id, id))
      .returning();
    return !!deleted;
  }
